c==== XXXXX check further array dims ?
c==== 
...
c
      subroutine dqagsONE(f,a1,b1,epsabs1,epsrel1,result1,abserr1,
     *   neval,ier,krule)
...
c
      common/INTEG1/lenw,limit,work(2001),iwork(500)
...
c
      ...
      l3 = limit+l2
c
      call dqagseONE(f,a,b,epsabs,epsrel,limit,result,abserr,neval,
...
10    if(ier.eq.6) lvl = 1
      if(ier.ne.0) call messageONE(ier)
c====
c==== 9/8/99 reset ier to avoid multi-dimension integral problems   RKH
c====
      ier=0
      return
      end
...
6     write(6,106)
      return
100   format(1x,'ERROR dqagsONE: integration invalid  ier =',i12)
101   format(1x,'ERROR dqagsONE: max no of sub-divisions')
102   format(1x,'WARN dqagsONE: round-off may underestimate integ. error')
103   format(1x,'WARN dqagsONE: bad integrand behaviour')
104   format(1x,'WARN dqagsONE: poor convergence')
105   format(1x,'ERROR dqagsONE: very poor convergence ')
106   format(1x,'ERROR dqagsONE: invalid input parameters ')
      end     
      subroutine dqagseONE(f,a,b,epsabs,epsrel,limit,result,abserr,neval,
...
c
      if(krule.eq.0)call dqk21ONE(f,a,b,result,abserr,defabs,resabs)
      if(krule.eq.1)call dqk31ONE(f,a,b,result,abserr,defabs,resabs)
      if(krule.eq.2)call dqk61ONE(f,a,b,result,abserr,defabs,resabs)
c
c           test on accuracy.
c
      dres = dabs(result)
...
c
        ...
        erlast = errmax
        if(krule.eq.0)then
        call dqk21ONE(f,a1,b1,area1,error1,resabs,defab1)
        call dqk21ONE(f,a2,b2,area2,error2,resabs,defab2)
        else if(krule.eq.1)then
        call dqk31ONE(f,a1,b1,area1,error1,resabs,defab1)
        call dqk31ONE(f,a2,b2,area2,error2,resabs,defab2)
        else if(krule.eq.2)then
        call dqk61ONE(f,a1,b1,area1,error1,resabs,defab1)
        call dqk61ONE(f,a2,b2,area2,error2,resabs,defab2)
        end if
c
...
c
      subroutine dqk21ONE(f,a,b,result,abserr,resabs,resasc)
...
c
   ...
   20 continue
     ...
     *  ((epmach*0.5d+02)*resabs,abserr)
      ...
      subroutine dqk31ONE(f,a,b,result,abserr,resabs,resasc)
...
c
   ...
   20 continue
     ...
     *  ((epmach*0.5d+02)*resabs,abserr)
      ...
      subroutine dqk61ONE(f,a,b,result,abserr,resabs,resasc)
