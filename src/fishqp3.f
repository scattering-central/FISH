...
c==== integrals. Don't forget to chnage the /INTEG1/ common block name !
c==== I don't think dqelg, dpsrt and d1mach need to be repeatd ?
c
      subroutine dqagsTHREE(f,a1,b1,epsabs1,epsrel1,result1,abserr1,
     *   neval,ier,krule)
c
...
c
      common/INTEG3/lenw,limit,work(2001),iwork(500)
...
c
      ...
      l3 = limit+l2
c
      call dqagseTHREE(f,a,b,epsabs,epsrel,limit,result,abserr,neval,
...
10    if(ier.eq.6) lvl = 1
      if(ier.ne.0) call messageTHREE(ier)
      ier=0
      return
      end
c
...
6     write(6,106)
      return
100   format(1x,'ERROR dqagsTHREE(: integration invalid  ier =',i12)
101   format(1x,'ERROR dqagsTHREE(: max no of sub-divisions')
c23456789012345678901234567890123456789012345678901234567890123456789012345678901234567890
102   format(1x,'WARN dqagsTHREE(: round-off underestimates integ. error ?')
103   format(1x,'WARN dqagsTHREE(: bad integrand behaviour')
104   format(1x,'WARN dqagsTHREE(: poor convergence')
105   format(1x,'ERROR dqagsTHREE(: very poor convergence ')
106   format(1x,'ERROR dqagsTHREE(: invalid input parameters ')
      end     
      subroutine dqagseTHREE(f,a,b,epsabs,epsrel,limit,result,abserr,neval,
...
c
      if(krule.eq.0)call dqk21THREE(f,a,b,result,abserr,defabs,resabs)
      if(krule.eq.1)call dqk31THREE(f,a,b,result,abserr,defabs,resabs)
      if(krule.eq.2)call dqk61THREE(f,a,b,result,abserr,defabs,resabs)
c
c           test on accuracy.
c
      dres = dabs(result)
...
c
        ...
        erlast = errmax
        if(krule.eq.0)then
        call dqk21THREE(f,a1,b1,area1,error1,resabs,defab1)
        call dqk21THREE(f,a2,b2,area2,error2,resabs,defab2)
        else if(krule.eq.1)then
        call dqk31THREE(f,a1,b1,area1,error1,resabs,defab1)
        call dqk31THREE(f,a2,b2,area2,error2,resabs,defab2)
        else if(krule.eq.2)then
        call dqk61THREE(f,a1,b1,area1,error1,resabs,defab1)
        call dqk61THREE(f,a2,b2,area2,error2,resabs,defab2)
        end if
c
...
c
      subroutine dqk21THREE(f,a,b,result,abserr,resabs,resasc)
...
c
   ...
   20 continue
     ...
     *  ((epmach*0.5d+02)*resabs,abserr)
      ...
      subroutine dqk31THREE(f,a,b,result,abserr,resabs,resasc)
...
c
   ...
   20 continue
     ...
     *  ((epmach*0.5d+02)*resabs,abserr)
      ...
      subroutine dqk61THREE(f,a,b,result,abserr,resabs,resasc)
